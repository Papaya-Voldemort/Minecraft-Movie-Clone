...
window.THREE = {
    ...
    WebGLRenderer: function(options) {
        ...
        // For now, force 2D context to avoid WebGL issues
        this.gl = null; // Disable WebGL for testing
        
        if (!this.gl) {
            // Fallback to 2D context
            this.context = this.domElement.getContext('2d');
            this.isWebGL = false;
            console.log('Using 2D canvas fallback (forced for compatibility)');
        } else {
            this.isWebGL = true;
        ...
        this.render = function(scene, camera) {
            if (this.isWebGL && this.gl) {
                ...
                // Draw some simple "blocks" to show the world
                this.context.fillStyle = '#8B4513'; // Brown blocks
                for (let i = 0; i < 5; i++) {
                    const x = 100 + i * 80;
                    const y = this.domElement.height * 0.7;
                    this.context.fillRect(x, y, 60, 60);
                }
                
                // Draw simple text to show it's working
                this.context.fillStyle = '#000000';
                this.context.font = 'bold 24px Arial';
                this.context.fillText('Minecraft Clone - Working!', 50, 50);
                this.context.font = '16px Arial';
                this.context.fillText('Use WASD to move, mouse to look around', 50, 80);
                this.context.fillText('Press E for inventory, C for crafting', 50, 105);
                this.context.fillText('Game is running in minimal 2D mode', 50, 130);
            }
        };
